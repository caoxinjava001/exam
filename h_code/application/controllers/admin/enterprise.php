...
class Enterprise extends MY_Controller{
    ...
    public function addUpdateInfo(){
		...
		//var_dump($user_info);exit;
        $data['id'] = $id;
        $this->rendering_admin_template($data,'enterprise','add_update');
    }
    ...
    public function updateAjax(){
		...
		//$where_user['control_type'] = INVEST;
		...
		$m_bx_one = $this->input->get_post('m_bx_one');//报税工资表年平均人数
		...
		$m_bx_two = $this->input->get_post('m_bx_two');//报税工资表年平均人数
		...
		$m_bx_three = $this->input->get_post('m_bx_three');//报税工资表年平均人数
        ...
        $field = 'cs_res';
        $is_file_val = $_FILES[$field];
        if ( isset($is_file_val) && $is_file_val['name'] ) {
            $f_config['upload_path'] = './statics/uploads/';
            $f_config['allowed_types'] = $this->_all_type;
            $f_config['encrypt_name'] = true;
            //$f_config['allowed_types'] = '*';
            $this->load->library('upload', $f_config);
            $u_ret = $this->upload->do_upload($field);
        
            if (!$u_ret) {
                $is_show_msg = $this->upload->display_errors();
                //$msg = trim($is_show_msg);
                //$msg = "请上传有效的生产经营信息表";
                //$ret["status"] = -1; 
                //$ret["msg"] = $msg;
                //createDomainAjax($ret["msg"], $ret["status"]);
            ...
            $upload_data_list =$this->upload->data();
            $file_path_sc = "/statics/uploads/".$upload_data_list["file_name"];
			}
        ...
        if ( isset($is_file_val) && $is_file_val['name'] ) {
            $f_config['upload_path'] = './statics/uploads/';
            $f_config['allowed_types'] = $this->_all_type;
            //$f_config['allowed_types'] = 'xlsx|doc|csv|docx';
            $f_config['encrypt_name'] = true;
            //$f_config['allowed_types'] = '*';
            $this->load->library('upload', $f_config);
            $u_ret = $this->upload->do_upload($field);
            ...
            $upload_data_list =$this->upload->data();
            $file_path_yy = "/statics/uploads/".$upload_data_list["file_name"];
			}
        ...
        if ( isset($is_file_val) && $is_file_val['name'] ) {
            ...
            $upload_data_list =$this->upload->data();
            $file_path_og = "/statics/uploads/".$upload_data_list["file_name"];
			}
        ...
        if ( isset($is_file_val) && $is_file_val['name'] ) {
            ...
            $upload_data_list =$this->upload->data();
            $file_path_xw = "/statics/uploads/".$upload_data_list["file_name"];
			}
        ...
        if ( isset($is_file_val) && $is_file_val['name'] ) {
            ...
            $upload_data_list =$this->upload->data();
            $file_path_other = "/statics/uploads/".$upload_data_list["file_name"];
			}
        }
		$one_info_list = array();
		...
		$three_info_list['m_bs'] = $m_bx_three;
		...
		if ($file_path_yy) {
		$update_info['yy_res'] = $file_path_yy;
		}
		if ($file_path_og) {
		$update_info['og_res'] = $file_path_og;
		}
		if ($file_path_xw) {
		$update_info['xw_res'] = $file_path_xw;
		}
		if ($file_path_sc) {
		$update_info['sc_res'] = $file_path_sc;
		}
		if ($file_path_other) {
		$update_info['other_res'] = $file_path_other;
		}
		...
		if ($user_info_tmp['zy_status'] == 0) {
			$update_info['zy_status'] = 1;
			$update_info['role_id']=0;
			$update_info['next_role_id']=ROLE_FOUR;
		}
		...
		if ($id) {
			...
			$where_manage .= ' and role_id in ('.ROLE_FOUR.','.ROLE_FIVE.','.ROLE_SIX.','.ROLE_SEVEN.') ';
    ...
    public function addAjax(){
		...
		$insert_info['yy_res'] = $file_path_yy;
		$insert_info['og_res'] = $file_path_og;
		$insert_info['xw_res'] = $file_path_xw;
		$insert_info['sc_res'] = $file_path_sc;
		$insert_info['other_res'] = $file_path_other;
		...
		$insert_info['next_role_id'] = ROLE_FOUR;
    ...
    public function index(){
		...
		// 只有业务员才可以处理
		if ($this->member_info['login_role_id'] == SALE_PERSONAL || $this->member_info['id'] == 1) {
			//拼装where条件
			if ($this->member_info['login_role_id'] == SALE_PERSONAL) {
		...
		$date = array();
        $this->rendering_admin_template($data,'enterprise','list');
    }
