...
class Xinsanban_Investigation_Info extends MY_Controller{
    ...
    public function __construct(){
        ...
        $this->load->model('xinsanban_investigation_info_model');
    ...
    public function init(){
        ...
        $order = 'status asc,id desc'; //根据需求: 冻结的公司往后放。
        $result=$this->xinsanban_investigation_info_model->list_info('*',$this->where,$this->page,$this->perpage,$order);
        //print_r($result);exit;
        ...
        $data['pages']=pages($this->xinsanban_investigation_info_model->getCount($this->where),$this->page,$this->perpage);

        //echo '<pre>';
        //print_r($data);exit;
        $this->rendering_admin_template($data,'xinsanban_investigation_info','list');
    }
    ...
    **************************************/
    public function  show(){
        ...
        if($id){
                $r = $this->xinsanban_investigation_info_model->get_one('*','id ='.$id);
                if($r){
                    $data['infos'] = $r;
                }else{
                    echo '查询结果 空！';
        ...
        $this->rendering_admin_template($data,'xinsanban_investigation_info','show');
    }
    ...
    ***************************************************************/
    public function ajax_isdel(){
        ...
        if($ids && $status){
            ...
            //print_r($param);exit;
            $r = $this->xinsanban_investigation_info_model->update($param,'id in('.$ids.')');
            if($r){
                    $return = array(
                        'status' => 1,
                        'msg' =>'更新成功！', 
